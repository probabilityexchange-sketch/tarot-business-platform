...
test.describe("Navigation User Flows", () => {
  ...
  test("can navigate to Readings page", async ({ page }) => {
    ...
    await Promise.all([
      page.waitForURL("**/readings"),
      page.getByRole("banner").getByRole("link", { name: "Readings" }).click(),
    ]);
  ...
  test("can navigate to Courses page", async ({ page }) => {
    ...
    await Promise.all([
      ...
      page.getByRole("banner").getByRole("link", { name: "Courses" }).click(),
    ]);
  ...
  test("can navigate to Journal/Blog page", async ({ page }) => {
    ...
    await Promise.all([
      ...
      page.getByRole("banner").getByRole("link", { name: "Journal" }).click(),
    ]);
